...
chessboard::chessboard (){
    for(int j=0;j<8;j++)
       ...
       head[2][j] = new empty_field(Nothing,2,j,_,0);
       head[3][j] = new empty_field(Nothing,3,j,_,0);
       head[4][j] = new empty_field(Nothing,4,j,_,0);
       head[5][j] = new empty_field(Nothing,5,j,_,0);
...
for(int i=0;i<8;i++)
    for(int j=0;j<8;j++)
        ...
        if(h.head[i][j]->fi==6)
           head[i][j]= new empty_field(Nothing,i,j,_,0);
        }
};
...
head[r->current_line][r->current_column]=new empty_field(Nothing,r->current_line,r->current_column,_,0);
...
head[line][column] = new empty_field(Nothing,line,column,_,0);
